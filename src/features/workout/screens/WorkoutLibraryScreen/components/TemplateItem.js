...
const TemplateItem = React.memo(({ item, expandedCards, onToggleExpanded, onEdit, onStart, onDelete, formatDate }) => {
  ...
  const gesture = Gesture.Pan()
    ...
    .onEnd((event) => {
      ...
      const shouldOpen = translation < -DELETE_BUTTON_WIDTH * 0.35 || velocity < -800;
      ...
      <View style={[styles.exerciseItem, isLastItem && styles.exerciseItemLast]}>
        <View style={styles.exerciseImageContainer}>
          {imageURL ? (
            ...
            <Ionicons name="barbell-outline" size={normalize(20)} color={styles.metaItemText.color} />
          )}
        </View>
    ...
    <View style={styles.swipeContainer}>
      <Animated.View style={[styles.deleteButtonBehind, deleteButtonStyle]}>
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Ionicons name="trash-outline" size={normalize(28)} color="#fff" />
