...
const WorkoutProgramLibrary = ({ 
  ...
  const handleEditSplit = useCallback((split) => {
    // Pass the full split object for editing
    navigation.navigate('CreateSplit', { 
      splitId: split.id, 
      splitData: split, // Pass the full object
      isEditing: true,
  ...
  const handleViewSchedule = useCallback((split) => {
    // Pass the full split object for direct viewing
  ...
  const renderItem = useCallback(({ item }) => (
    ...
    />
  ), [expandedCards, toggleExpanded, handleEditSplit, onActivateSplit, handleViewSchedule, activeSplitId]);
  ...
  const flatListProps = useMemo(() => ({
    ...
    // Note: getItemLayout is for performance with fixed-size items. 
    // You may need to adjust the length based on your ProgramItem component's height.
