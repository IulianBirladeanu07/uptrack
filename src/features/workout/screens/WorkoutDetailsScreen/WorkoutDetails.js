...
const WorkoutDetails = ({ route, navigation }) => {
    const { duration, exercises, timestamp, workoutName } = route.params;
    ...
    return (
        <View style={styles.container}>
            ...
            <ViewShot ref={workoutCardRef} style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: colors.background.primary }}>
                <View style={styles.backgroundContainer}>
                    {selfieUri ? (
                        <Image source={{ uri: selfieUri }} style={styles.backgroundImage} />
                    ) : (
                        <LinearGradient colors={['#1a1a1a', colors.background.primary]} style={styles.backgroundGradient} />
                    )}
                    ...
                    <Animated.View style={[styles.workoutCard, animatedStyle]}>
                        ...
                        <View style={styles.heroSection}>
                            <Text style={styles.heroTitle}>
                                {workoutName ? workoutName.toUpperCase() : 'WORKOUT\nCOMPLETE'}
                            </Text>
            ...
            {!isSharing && (
                <View style={[styles.controlsContainer, { paddingBottom: insets.bottom + 10 }]}>
                    <TouchableOpacity style={styles.primaryBtn} onPress={handleShare}>
                        ...
                        >
                            <Ionicons name="logo-instagram" size={20} color={colors.accent.buttonText} />
